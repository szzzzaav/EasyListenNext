...
const useMusicObjects = ({
  data,
}: UseMusicObjectsProps) => {
  const {
    dispatch,
    volumnRef,
    progressRef,
    ...
    const getData = async () => {
      ...
      try {
        ...
        const songWorker = new Worker(
          new URL(
            "@/workers/loadAudioWorker.ts",
            import.meta.url
          )
        );
        const lyricWorker = new Worker(
          new URL(
            "@/workers/loadLyricWorker.ts",
            import.meta.url
          )
        );
        ...
        const [
          ...
          loadLyric(
            lyricWorker,
            lyricUrl.data.publicUrl
          ),
        ]);

        const newMusicObject =
          await new AudioControlObject(
        ...
        newMusicObject.register({
          showlryonlrcstage:
            newLyric.showlryonlrcstage.bind(
              newLyric
            ),
        });
